...
export const CrossChainFlow: React.FC<CrossChainFlowProps> = ({ status }) => {
  return (
    <div className="border border-green-500/30 bg-black p-6 rounded-lg font-mono">
      ...
      <div className="flex items-center justify-between relative px-4">
        ...
        <div className={`flex flex-col items-center gap-2 ${['burning', 'messaging', 'minting', 'completed'].includes(status) ? 'text-green-400' : 'text-green-900'}`}>
          <div className={`w-12 h-12 border border-current flex items-center justify-center rounded ${status === 'burning' ? 'animate-pulse' : ''}`}>BASE</div>
        ...
        <div className={`flex flex-col items-center gap-2 ${['minting', 'completed'].includes(status) ? 'text-green-400' : 'text-green-900'}`}>
          <div className={`w-12 h-12 border border-current flex items-center justify-center rounded ${status === 'minting' ? 'animate-pulse' : ''}`}>OP</div>
      ...
      <div className="mt-8 text-[11px] text-green-500/70 h-4">
        ...
        {status === 'burning' && "> Executing ICrosschainERC20.crosschainBurn()..."}
